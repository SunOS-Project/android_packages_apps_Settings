...
public class DataSaverBackend {
    ...
    public void setIsAllowlisted(int uid, String packageName, boolean allowlisted) {
        ...
        if (allowlisted) {
            mDynamicDenylistManager.setUidPolicyLocked(uid, POLICY_ALLOW_METERED_BACKGROUND);
            mMetricsFeatureProvider.action(
                    mContext, SettingsEnums.ACTION_DATA_SAVER_WHITELIST, packageName);
        } else {
            mDynamicDenylistManager.removeUidPolicyLocked(uid, POLICY_ALLOW_METERED_BACKGROUND);
        }
        mDynamicDenylistManager.removeUidPolicyLocked(uid, POLICY_REJECT_METERED_BACKGROUND);
    }
    ...
    public void setIsDenylisted(int uid, String packageName, boolean denylisted) {
        ...
        mUidPolicies.put(uid, policy);
        if (denylisted) {
            mDynamicDenylistManager.setUidPolicyLocked(uid, POLICY_REJECT_METERED_BACKGROUND);
            mMetricsFeatureProvider.action(
